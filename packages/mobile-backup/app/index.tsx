...
export default function HomeScreen() {
  ...
  const handleCreateGame = async () => {
    if (!isFirebaseConfigured || !auth?.currentUser) {
      Alert.alert("Firebase Config Missing", firebaseConfigError ?? "Firebase is not configured.");
      return;
    }
  ...
  return (
    <View style={styles.container}>
      ...
      {!isFirebaseConfigured && (
        <Text style={styles.warning}>{firebaseConfigError}</Text>
      )}
...
const styles = StyleSheet.create({
  ...
  warning: { color:"#ff9aa2", textAlign:"center", marginBottom:16 },
});
